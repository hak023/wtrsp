...
enum e_max_codec {
   ...
   E_CODEC_MAX_SDP_TRANSCODING = 20, // R132 PKG Add by hak. TransCoding 정책 최대 갯수.
};
...
//for R132 PKG add by hak Start.
struct st_sdp_transcoding : public st_as_base {
   enum { e_maxnum_name=128, e_maxnum_value=128, e_maxmum_fmtp=256 };
   enum e_rtcp_type {
	   E_RTCP_BYPASS = 1,
	   E_RTCP_DROP = 2
   };

	int nID; // SDP TransCoding 의 ID. Essential.
	char szName[e_maxnum_name]; // Name. Essential.
	char szRID[e_maxnum_value]; //SDP Rule의 ID를 사용, 다중 RuleID 입력으로 char 형으로 변경함. Essential.

	int nAddCodecID; // Add Codec ID. DIS-SDP-CODEC을 참조함. Essential.
	int nRTCP;							// e_rtcp_type 참조. BYPASS:1 or DROP:2.
	int nTransCodingOn; // ON, OFF Flag. Essential.
	char szReason[e_maxnum_name];	//Response시에 실패로 응답할 경우 이유를 세팅하여 전송함. Optional.

	char *str(char *pBuf, int nLen)
	{
		snprintf(pBuf, nLen, "ID=%d, Name=%s, RID=%s, AddCodecId=%d, nRTCP=%d, ON=%d, Reason=%s"
									, nID, szName, szRID, nAddCodecID, nRTCP, nTransCodingOn, szReason);
		return pBuf;
	}
};
//for R132 PKG add by hak End.
...
struct st_as_shm {
	...
	st_sdp_transcoding stSdpTransCoding[E_CODEC_MAX_SDP_TRANSCODING]; // for R132 PKG add by hak.
};
