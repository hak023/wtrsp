...
private:
   ...
   ///session vector
   typedef std::vector<pool_element_ptr>  ses_vec;

public:
   ///constructor
   cuser_pool()
      : m_nInitMaxCnt(T_InitSize),
        m_pSesVec(new ses_vec),
   ...
   int GetPoolSize(){
      return m_pSesVec->size();
   }
   pool_element_ptr GetElement(int index){
      return (pool_element_ptr)((*m_pSesVec)[index]);
   }
protected:
   int               m_nInitMaxCnt;
   ses_vec*          m_pSesVec;
