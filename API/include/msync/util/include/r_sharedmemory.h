...
#ifndef	__RPDK_GSHM_CLASS_H__
#define	__RPDK_GSHM_CLASS_H__
...
#ifndef WIN32
//mod yun 20151002  change data type int -> size_t
//inline void *CreateShm(LPCSTR pszPath, int size, int fRdOnly = 0)
inline void *CreateShm(LPCSTR pszPath, size_t size, int fRdOnly = 0)
{
	key_t key;
	...
	if ((key = ftok(pszPath, FTOK_ID)) < 0) {
		//LOGMGR(CLogMgr::Error,
            //"[CreateShm] ftok(%s) error(%s)!", 
	...
	if ((nShmId = shmget(key, 0, 0)) < 0) {
		if (errno != ENOENT) {
			LOGMGR(CLogMgr::Error,
               "[CreateShm] 1 shmget() failed(%s)!", strerror(errno));
			return NULL;
		}
		if ((nShmId = shmget(key, size, SHM_MODE | IPC_CREAT | IPC_EXCL)) < 0) {
			LOGMGR(CLogMgr::Error,
               "[CreateShm] 2 shmget() failed(%s)!", strerror(errno));
			return NULL;
		}
	}
	else {
		nOldShmId = nShmId;
		if ((nShmId = shmget(key, size, SHM_MODE)) < 0) {
			LOGMGR(CLogMgr::Error,
               "[CreateShm] 3 shmget() failed(%s) with old id(%d)!",
			      strerror(errno), nOldShmId);
			return NULL;
		}
	}
	...
	if ((PDKLONG)pShm == -1) {
		//LOGMGR(CLogMgr::Error,
            //"[CreateShm] shmat() failed(%s) with id(%d)!",
	         //strerror(errno), nShmId);
		return NULL;
	}

	//LOGMGR(CLogMgr::Info,
         //"[CreateShm:%d] created with the token \"%s\".",
         //nShmId, pszPath);
	return pShm;
}

inline int DestroyShm(LPCSTR pszPath)
{
	key_t key;
	int nShmId;
	...
	if ((key = ftok(pszPath, FTOK_ID)) < 0) {
		//LOGMGR(CLogMgr::Error,
            //"[DestroyShm] ftok() error(%s)!", strerror(errno));
		return -1;
	}
	...
	if ((nShmId = shmget(key, 0, SHM_MODE)) < 0) {
		if (errno == ENOENT) {
			//LOGMGR(CLogMgr::Error,
               //"[DestroyShm] No such shm segment(%s)!",
			      //strerror(errno));
			return 1;
		}

		//LOGMGR(CLogMgr::Error,
            //"[DestroyShm] shmget() failed(%s)!", strerror(errno));
		return -1;
	}

	if (shmctl(nShmId, IPC_RMID, 0) < 0) {
		//LOGMGR(CLogMgr::Error,
            //"[DestroyShm] shmctl() failed(%s)!", strerror(errno));
		return -1;
	}

	//LOGMGR(CLogMgr::Info,
         //"[DestroyShm] destroyed with the token \"%s\".", pszPath);
	return 1;
}

#else   //if WIN32
inline void *CreateShm(LPCSTR pszShmName, int size, int fRdOnly = 0)
{
	HANDLE  hShm;
	DWORD   dwShmAccessMode = FILE_MAP_ALL_ACCESS;
	void * pShm;


	hShm = CreateFileMapping((HANDLE)0xFFFFFFFF, NULL, PAGE_READWRITE, 0, size, pszShmName);
    if(!hShm) {
		//LOGMGR(CLogMgr::Error,
            //"[CreateShm] CreateFileMapping(%s) failed(%d)!", pszShmName, GetLastError());
		return NULL;
    }

	if(fRdOnly) {
		dwShmAccessMode = FILE_MAP_READ;
	}
	pShm = MapViewOfFile(hShm, dwShmAccessMode, 0, 0, size);
    if(pShm== NULL) {
       CloseHandle(hShm);
	   hShm = NULL;
    }
...
inline int DestroyShm(LPCSTR pszShmName)
{
	HANDLE  hShm;
	hShm = OpenFileMapping(PAGE_READWRITE, true, pszShmName);
    if(!hShm) {
		//LOGMGR(CLogMgr::Error,
            //"[DestroyShm] OpenFileMapping(%s) failed(%d)!", pszShmName, GetLastError());
		return -1;
    }

	CloseHandle(hShm);
	//LOGMGR(CLogMgr::Info,
         //"[DestroyShm] destroyed with the token \"%s\".", pszShmName);
	return 1;
}
#endif //WIN32
	...
	int Initialize(int key)
	{
		...
		if ((m_nShmId = shmget(key, m_nSize, 0)) < 0) {
			if (errno != ENOENT) {
				...
				throw CXException("shmget() failed %d:%s", errno, strerror(errno));
			}
			if ((m_nShmId = shmget(key,m_nSize,SHM_MODE|IPC_CREAT|IPC_EXCL)) < 0) {
				perror("shmget() failed");
				throw CXException("shmget() failed");
			}
			m_bOrigin = false;
		} else {
			/* int nOld = nShmId; */
			if ((m_nShmId = shmget(key, m_nSize, SHM_MODE)) < 0) {
				...
				throw CXException("shmget() failed with old id");
			}
			m_bOrigin = true;
		}
		Attach();
...
public:
	...
	T *Attach(int shmflg = 0)
	{
		...
		if (m_pMutexId == SHM_FAILED)
			perror("CSharedMemory Attach");
		m_pBuffer = (T *)((PDKULONG)m_pMutexId + sizeof(pthread_mutex_t));
		if (!m_bOrigin) {
...
public:
   ...
   virtual bool AddEvent(PDK16U type, PDK16U stype, char *pValue = NULL, int nLen = 0);
	virtual bool AddEvent(PDK16U type, PDK16U stype, PDKULONG value);
...
template <class T>
inline bool CShmQInterface<T>::AddEvent(PDK16U type, PDK16U stype, PDKULONG value)
{
#if 0
   T Event;
   T *pEvent = &Event;
   ...
   EVENTINFO_TYPE(pEvent) = type;
   EVENTINFO_SUBTYPE(pEvent) = stype;
   *(PDKULONG *)(&pEvent->Value[0]) = value;
   EVENTINFO_LENGTH(pEvent) = EVENTINFO_HEADER_LENGTH + sizeof(PDKULONG);
   return Add(pEvent);
#else
	return true;
#endif
...
////////////////////////////////////////////////////////////////////////////////
template <class T>
class DLLPORT CShmQThread : public CShmQInterface<T>, public CThread
{
private:
...
template <class T>
class DLLPORT CShmTrace : public CThread
{
protected:
	typedef struct {
		sem_t		m_Semaphore;
