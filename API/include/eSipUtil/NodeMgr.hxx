...
class NodeLoc : public StlObject
{
	public:
		...
		*/
		...
		unsigned int m_eType;    // udp=0/tcp=1/tls=2
...
#ifdef DEF_NODEMGR_ARTE_ADDED
class NodeArte : public StlObject
{
	public:
		NodeArte(NodeRte * _pclsOwner){m_pclsOwner = _pclsOwner;m_unRteId=0;}
		~NodeArte(){}
...
class NodeRte : public StlObject
{
	public:
		...
		*/
		...
		const char * m_fnFindExtParam(const char * _pszKey);
		#ifdef DEF_NODEMGR_ARTE_ADDED
		bool m_fnAddArte(unsigned int _unRteID);
		unsigned int m_fnDecisionArte(Net5Tuple_t & _rstAddr);
		void m_fnBuildArte(KString &_rclsBuild,unsigned int _unDepth);
		#endif
		RwMutex m_clsLock;
		unsigned int m_unID;
		KString m_clsName;
