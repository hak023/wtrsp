...
#else  // DEF_RTE_R131
snprintf(_pBuf,_nLen,
         "ID:%u, DESC:%s, LB(%d/ON:%d, %d/ON:%d), CM(%d/ON:%d, %d/ON:%d), RSP_ID:%d, C(%d/%d/%d/%d), C:%d,%d U/I/D:%u/%d/%d NPDB:%u, HLR:%u",
         m_uiID,m_szDesc,
         m_nMaxCPS_LB,m_nCtrlCPSOn_LB,m_nMaxTPS_LB,m_nCtrlTPSOn_LB,
         m_nMaxCPS,m_nCtrlCPSOn,m_nMaxTPS,m_nCtrlTPSOn,
         m_nRspID,m_nChargingOn,m_nChargingCode,m_nChargingIDC,m_nRoamingIDC,
         m_nCurCPS,m_nCurTPS, m_ucUsed,m_nIndex,m_ucInterDel,m_nNpdbEnable, m_nHlrEnable);
#endif
#endif
      return _pBuf;
   }
...
struct SipReasonCode_t
{
   ...
   enum ECauseIndex_t
   {
	...
	E_CAUSE_MAX                     =18,
	#else   // new code
	E_NPDB_STS_DOWN_CAUSE = 18,
	E_HLR_STS_DOWN_CAUSE = 19,
	E_CAUSE_MAX = 20
	#endif
   };
