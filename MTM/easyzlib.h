...
struct ezbuffer
{
	...
	~ezbuffer() { Release(); };
	unsigned char* Alloc( int n ) { if (nSize<n) {Release(); pBuf=new unsigned char[n]; nSize = n; memset(pBuf,0x00,nSize);} nLen=n; return pBuf; };
	void Release() { if (pBuf) { memset(pBuf, 0x00, nSize); delete[] pBuf; Init(); } };
	unsigned char* pBuf;
