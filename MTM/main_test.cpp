...
/*
  ...
  // int res = KString::m_fnStrCaseCmp((KSTR)clsTcCommand, (KSTR)clsTest);
   ...
   KString clsTemp = "sdfsdf.vpm_tmp";
   if(KString::m_fnStrStr((KSTR)clsTemp, "_tmp") != NULL)
   {
      //printf("name: %s\n", (KCSTR)clsTemp);
      //KString clsTemp2 = KString::m_fnTrimTailString((KSTR)clsTemp, "_tmp");
      KString clsTemp2 = KString::replaceAll((KCSTR)clsTemp, ".vpm_tmp", ".vpm");
      printf("name: %s\n", (KCSTR)clsTemp);
      printf("name22: %s", (KCSTR)clsTemp2);
   }


   std::system("chmod 777 ./test.txt");
   return 0;
   KString clsTest1="40";
   KString clsTest2="40K";
   KString clsTest3="40KB";
   KString clsTest4="";

   KString clsResult;
   clsResult ="";
   clsResult << (KINT)clsTest1 << "K";
   std::cout << (KCSTR)clsResult << std::endl;

   clsResult ="";
   clsResult << (KINT)clsTest2 << "K";
   std::cout << (KCSTR)clsResult << std::endl;

   clsResult ="";
   clsResult << (KINT)clsTest3 << "K";
   std::cout << (KCSTR)clsResult << std::endl;

   clsResult ="";
   printf("%d", clsTest4.m_unRealLen);
   clsResult << (KINT)clsTest4 << "K";
   std::cout << (KCSTR)clsResult << std::endl;
...
/*************/
/*
   KString clsTcBinary = "/home/ibc/ibc/src/vIBC/trunk/src/TCM/PYTHON37/bin/python3 /home/ibc/ibc/src/vIBC/trunk/src/TCM/commandTimeoutTest.py 45 ffmpeg -i /home/ibc/music_original5/vwtrss/audio/01000034289719.mp3 -f mp3 -acodec mp3 -ac 2 -ar 44100 -ab 192000 -y /home/ibc/music/mig/vwtrss/test/MP3TestFile_1.mp3 2>&1";
   FILE *fp;
   ...
   KString clsBuff;  clsBuff.m_fnReSize(1024);
   while(fgets((KSTR)clsBuff, 1024, fp))
   {
      printf(">>> %s", (KCSTR)clsBuff);
   }
   pclose(fp);
   return 0;
*/
