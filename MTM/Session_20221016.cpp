...
Session::~Session()
{
   ...
   if (m_nAllocWorker != -1)
   {
      ...
      //int nTotalSessionNum = (MainConfig::m_fnGetInstance()->m_unWorkerNum) * (MainConfig::m_fnGetInstance()->m_unWorkerIdMgrNum);
      int nTotalSessionNum = (MainConfig::m_fnGetInstance()->m_unWorkerIdMgrNum);
      IFLOG(E_LOG_ERR, "Del Session[Total Session:%d]", nTotalSessionNum -Worker::m_pWorkerIdMgr->getAvailableIdNum());
   } 
}
...
//			IFLOG(E_LOG_DEBUG, "SourceContainerID:[%s], TargetContainerID:[%s], VideoCodec:[%s], AudioCodec:[%s]", (KSTR)clsSourceContainerID, (KSTR)clsTargetContainerID, (KSTR)clsTargetVideoCodec, (KSTR)clsTargetAudioCodec);
			...
			if (pclsTarget->m_eTcSt == E_TC_RES_OK)
			{
				...
				if (bFind)
				{
					...
					{
						// 예외
						IFLOG(E_LOG_ERR, "Unavailable Transcoding Tool(ONLY FFMPEG/MCONV)->E_TRANSCODING_FAILED");
...
void Session::m_fnProcTrsgTranscoding()
{
	...
	while (pclsTarget)
	{
		...
		{
//			pclsTarget->m_eTcSt = m_fnExecuteTranscoding(pclsTarget->m_clsTcCommand, pclsTarget->m_clsTcType, m_pclsSourceContent->m_clsID, pclsTarget->m_clsContainer.m_clsID, clsResultDescription, pclsTarget->m_clsOutputFile);
...
//					pclsTarget->m_eTcSt = m_fnExecuteTranscoding(clsVoxCommand, pclsTarget->m_clsTcType, m_pclsSourceContent->m_clsID, pclsTarget->m_clsContainer.m_clsID, clsResultDescription, pclsTarget->m_clsOutputFile);
...
KString Session::m_fnGetFileName()
{
	...
	KString::m_fnStrnCat((KSTR) clsFileName, clsFileName.m_unLen,
			"%04d%02d%02d/%04d%02d%02d%02d%02d%02d%08lld_encode.tmp",
			tmTime.tm_year + 1900, tmTime.tm_mon + 1, tmTime.tm_mday,
			tmTime.tm_year + 1900, tmTime.tm_mon + 1, tmTime.tm_mday,
			tmTime.tm_hour, tmTime.tm_min, tmTime.tm_sec, now_monotonic.tv_nsec / 10);

	KString clsDir;
	KString::m_fnStrnCat((KSTR) clsDir, clsDir.m_unLen, "%s/%04d%02d%02d", (KCSTR)MainConfig::m_fnGetInstance()->m_clsNasInternal,
	         tmTime.tm_year + 1900, tmTime.tm_mon + 1, tmTime.tm_mday);
...
ETrssCodeSet_t Session::m_fnExecuteTranscoding(KString &_rclsTcCommand, ETcType_t _eTcType, KString &_rclsSourceContainerID, KString &_rclsTargetContainerID, KString & _rclsResultDescription, KString &_rclsOutputFile)
{
	MainConfig *pclsConf = MainConfig::m_fnGetInstance();
	KString clsTcBinary; clsTcBinary.m_fnReSize(BUFF_SIZE);

//	clsTcBinary << "timeout " << pclsConf->m_unCmdTimeout << "s " << (KCSTR)_rclsTcCommand << " 2>&1 &";
	...
	if(fp == NULL)
	{
		IFLOG(E_LOG_ERR, "[S:%s][T:%010u][J:%s]Execute Command(%s) FAILED->E_TRANSCODING_FAILED", (KCSTR)m_clsSessionID, m_unTid, (KCSTR)m_clsJobID, (KSTR)clsTcBinary);
		pclose(fp);
	...
	if(_eTcType == E_TC_FFMPEG)
	{
		while(fgets((KSTR)clsBuff, BUFF_SIZE, fp))
		{
			...
			if(KString::m_fnStrStr((KSTR)clsBuff, "WTRSP_FileSize:") != NULL)
			{
				...
				IFLOG(E_LOG_INFO, "[S:%s][T:%010u][J:%s] %s ResultFileSize: %s", (KCSTR)m_clsSessionID, m_unTid, (KCSTR)m_clsJobID, (KCSTR)g_fnGetTranscodingType(_eTcType), (KCSTR)clsSize);
			...
			{
				IFLOG(E_LOG_ERR, "[S:%s][T:%010u][J:%s] %s_TC_FAILED: %s", (KCSTR)m_clsSessionID, m_unTid, (KCSTR)m_clsJobID, (KCSTR)g_fnGetTranscodingType(_eTcType), (KSTR)clsBuff);
			...
			{
				IFLOG(E_LOG_ERR, "[S:%s][T:%010u][J:%s] %s_TC_FAILED: Received Signal Command TimeOut", (KCSTR) m_clsSessionID, m_unTid, (KCSTR) m_clsJobID, (KCSTR) g_fnGetTranscodingType(_eTcType));
			...
			{
				IFLOG(E_LOG_ERR, "[S:%s][T:%010u][J:%s] %s_PERMISSON_FAILED", (KCSTR) m_clsSessionID, m_unTid, (KCSTR) m_clsJobID, (KCSTR) g_fnGetTranscodingType(_eTcType));
		...
		{
			IFLOG(E_LOG_ERR, "[S:%s][T:%010u][J:%s] %s_TC_FAILED: ResultFile is Empty", (KCSTR) m_clsSessionID, m_unTid, (KCSTR) m_clsJobID, (KCSTR) g_fnGetTranscodingType(_eTcType));
		...
		else
		{
			...
			IFLOG(E_LOG_INFO, "[S:%s][T:%010u][J:%s] %s_TC_SUCCESS Src:%s->Tgt:%s ExecutionTime:%.3lf(ms), ResultPlayTime:%d", (KCSTR) m_clsSessionID, m_unTid, (KCSTR) m_clsJobID, (KCSTR) g_fnGetTranscodingType(_eTcType), (KCSTR) _rclsSourceContainerID, (KCSTR) _rclsTargetContainerID, timeDiffMs, (KUINT) _rclsResultDescription);

			KString clsChgMod = "chmod 777 ";
			// FCONV로 변환하는 VOX는 중간파일로 TempFile 변경
			if(_rclsTargetContainerID == "VOX")
				clsChgMod << (KCSTR)_rclsOutputFile;
			clsChgMod << (KCSTR)_rclsOutputFile;
	...
	else	// MCONV, VOX
	{
		while(fgets((KSTR)clsBuff, BUFF_SIZE, fp))
		{
			...
			else if (KString::m_fnStrStr((KSTR) clsBuff, "Code 0,") != NULL)
			{
				...
				clock_gettime(CLOCK_MONOTONIC, &endTimespec);
				timeDiffMs = (endTimespec.tv_sec - stTimespec.tv_sec) * 1000 + (endTimespec.tv_nsec - stTimespec.tv_nsec)/1000000.0;
				IFLOG(E_LOG_INFO, "[S:%s][T:%010u][J:%s] %s_TC_SUCCESS Src:%s->Tgt:%s ExecutionTime:%.3lf(ms), ResultPlayTime:%d", (KCSTR)m_clsSessionID, m_unTid, (KCSTR)m_clsJobID,
								(KCSTR)g_fnGetTranscodingType(_eTcType), (KCSTR)_rclsSourceContainerID, (KCSTR)_rclsTargetContainerID, timeDiffMs, (KUINT)_rclsResultDescription);
			...
			{
				IFLOG(E_LOG_ERR, "[S:%s][T:%010u][J:%s] PERMISSON_FAILED", (KCSTR) m_clsSessionID, m_unTid, (KCSTR) m_clsJobID);
